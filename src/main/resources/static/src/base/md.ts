 ...
 */
export function hex_md(s: string): string {
...
class MD5CodeMaker {
     ...
     */
    static core_md5(x: Array<number>, len: number) {
        /* append padding */
        x[len >> 5] |= 0x80 << ((len) % 32);
        x[(((len + 64) >>> 9) << 4) + 14] = len;
    ...
    static md5_ii(a: number, b: number, c: number, d: number, x: number, s: number, t: number) {
     ...
     */
    static str2binl(str: string): Array<number> {
        let bin = new Array<number>();
     ...
     */
    static binl2hex(binarray: Array<number>): string {
