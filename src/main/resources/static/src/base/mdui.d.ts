...
interface jQueryObjectsOperations {
	/** 遍历数组 */
	each<T>(x: Array<T>, iterator: (i: number, value: T) => void):jQueryObjectsOperations
	/** 遍历对象 */
	each(x: Object, iterator: (i: number, value: any) => void):jQueryObjectsOperations
...
interface MduiDialogs {
	...
	prompt(label: string, onConfirm?: (v: string, e: MduiDialogsInstance) => void,
		onCancel?: (v: string, e: MduiDialogsInstance) => void, options?: MduiDialogsPromptOptions):any
	/** 打开一个提示用户输入的对话框 */
